...
Company.prototype.createForm = function(wrapper) {
    ...
    wrapper.innerHTML = `
        <form class="form-group company-form" ${id ? `data-id=${id}` : ''}>
            <input type="text" class="form-control" ${name ? `value="${name}"` : ''} name="name" 
                placeholder="Name: starts with capital letter">
            ${errorDiv}
            <input type="number" class="form-control" ${employeesAmount ? `value="${employeesAmount}"` : ''} 
            ...
            ${errorDiv}
            <input type="text" class="form-control" ${officialSite ? `value="${officialSite}"` : ''} name="officialSite" 
