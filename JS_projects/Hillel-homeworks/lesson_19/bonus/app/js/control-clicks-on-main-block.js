    ...
    function handleClickOnMainBlock(e) {
        ...
        function handleClickOnConfirmFormBtn() {
            ...
            function updateItem() {
                ...
                const itemTimeOfRegistration = itemDataArr[itemIndex].timeOfRegistration || null;
                ...
                const newItemObj = createItemAccordingToCondition(className, itemId, ...formData,
                    itemTimeOfRegistration, itemBalance);
        ...
        function handleClickOnBuyOrCellCarBtn(identifier, selectClassName) {
            const targetedElementParent = targetedElement.parent();
            const select = targetedElementParent.find(`select${selectClassName}`);
            const errorBlock = targetedElementParent.find(`.error-message${selectClassName}`);

            if (!select[0].value) {
                showErrorMessage(errorBlock, 'Please, select car/car\'s first');
                return;
            }
            ...
            if (identifier === 'buy') {
                ...
                if (totalOrderValue > ownerBalance) {
                    showErrorMessage(errorBlock, 'Not enough money for this order(');
                    return;
                }
